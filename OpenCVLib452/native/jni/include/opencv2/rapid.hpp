 ...
 */
        CV_EXPORTS_W void
        drawSearchLines(InputOutputArray img, InputArray locations, const Scalar &color);
 ...
 */
        CV_EXPORTS_W void drawWireframe(InputOutputArray img, InputArray pts2d, InputArray tris,
                                        const Scalar &color, int type = LINE_8,
 ...
 */
        CV_EXPORTS_W void
        extractControlPoints(int num, int len, InputArray pts3d, InputArray rvec, InputArray tvec,
 ...
 */
        CV_EXPORTS_W void
        extractLineBundle(int len, InputArray ctl2d, InputArray img, OutputArray bundle,
 ...
 */
        CV_EXPORTS_W void
        convertCorrespondencies(InputArray cols, InputArray srcLocations, OutputArray pts2d,
 ...
 */
        CV_EXPORTS_W float
        rapid(InputArray img, int num, int len, InputArray pts3d, InputArray tris, InputArray K,
...
/// Abstract base class for stateful silhouette trackers
        ...
        compute(InputArray img, int num, int len, InputArray K, InputOutputArray rvec,
                InputOutputArray tvec,
                const TermCriteria &termcrit = TermCriteria(
                        TermCriteria::MAX_ITER | TermCriteria::EPS, 5, 1.5)) = 0;

        CV_WRAP virtual void clearState() = 0;
    };

/// wrapper around @ref rapid function for uniform access
    class CV_EXPORTS_W Rapid

    : public Tracker {
    public:
    CV_WRAP static Ptr<Rapid>
    create(InputArray
    pts3d,
    InputArray tris
    );
 ...
 */
class CV_EXPORTS_W OLSTracker

: public Tracker
{
public:
CV_WRAP static Ptr<OLSTracker>
create(InputArray
pts3d,
InputArray tris,
int histBins = 8, uchar
sobelThesh = 10
);
};
//! @}
