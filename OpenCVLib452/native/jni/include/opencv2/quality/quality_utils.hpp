...
// convert inputarray to specified mat type.  set type == -1 to preserve existing type
            template<typename R>
            inline R extract_mat(InputArray
            in,
            const int type = -1
            ) {
            R result = {};
...
// extract and expand matrix to target type
        template<typename R>
        inline R expand_mat(InputArray
        src,
...
inline void scale(cv::Mat &mat, const cv::Mat &range, const T min, const T max) {
    ...
    for (int y = 0; y < mat.rows; ++y) {
        ...
        for (int x = 0; x < mat.cols; ++x)
            row.at<T>(x) = min + (max - min) * (row.at<T>(x) - row_min.at<T>(x)) /
                                 (row_max.at<T>(x) - row_min.at<T>(x));
    }
}
