...
//! @{
        ...
        CV_WRAP static Ptr<RgbdNormals>

        create(int rows, int cols, int depth, InputArray K, int window_size = 5, int method =
        RgbdNormals::RGBD_NORMALS_METHOD_FALS);
        ...
        void
        initialize_normals_impl(int rows, int cols, int depth, const Mat &K, int window_size,
                                int method) const;
    ...
    CV_WRAP static Ptr<DepthCleaner>

    create(int depth, int window_size = 5, int method = DepthCleaner::DEPTH_CLEANER_NIL);
...
CV_EXPORTS_W
void
registerDepth(InputArray unregisteredCameraMatrix, InputArray registeredCameraMatrix,
...
CV_WRAP static Ptr<RgbdPlane>

create(int method, int block_size, int min_size, double threshold,
...
OdometryFrame(const Mat &image, const Mat &depth, const Mat &mask = Mat(),
              const Mat &normals = Mat(), int ID = -1);

CV_WRAP static Ptr<OdometryFrame>

create(const Mat &image = Mat(), const Mat &depth = Mat(), const Mat &mask = Mat(),
       const Mat &normals = Mat(), int ID = -1);
 ...
 */
CV_WRAP bool
compute(const Mat &srcImage, const Mat &srcDepth, const Mat &srcMask, const Mat &dstImage,
        const Mat &dstDepth,
...
CV_WRAP_AS(compute2)
bool
        compute(Ptr < OdometryFrame > &srcFrame, Ptr < OdometryFrame > &dstFrame, OutputArray
Rt,
...
prepareFrameCache(Ptr<OdometryFrame>
& frame,
int cacheType
) const;

CV_WRAP static Ptr<Odometry>

create(const String &odometryType);

/** @see setCameraMatrix */
CV_WRAP virtual cv::Mat
...
virtual bool
computeImpl(const Ptr <OdometryFrame> &srcFrame, const Ptr <OdometryFrame> &dstFrame,
            OutputArray Rt,
 ...
 */
class CV_EXPORTS_W RgbdOdometry
 ...
 */
RgbdOdometry(const Mat &cameraMatrix, float minDepth = Odometry::DEFAULT_MIN_DEPTH(),
...
CV_WRAP static Ptr<RgbdOdometry>

create(const Mat &cameraMatrix = Mat(), float minDepth = Odometry::DEFAULT_MIN_DEPTH(),
       float maxDepth = Odometry::DEFAULT_MAX_DEPTH(),
       float maxDepthDiff = Odometry::DEFAULT_MAX_DEPTH_DIFF(),
       const std::vector<int> &iterCounts = std::vector<int>(),
       const std::vector<float> &minGradientMagnitudes = std::vector<float>(),
...
getMinGradientMagnitudes() const {
 ...
 */
class CV_EXPORTS_W ICPOdometry
 ...
 */
ICPOdometry(const Mat &cameraMatrix, float minDepth = Odometry::DEFAULT_MIN_DEPTH(),
            float maxDepth = Odometry::DEFAULT_MAX_DEPTH(),
            float maxDepthDiff = Odometry::DEFAULT_MAX_DEPTH_DIFF(),
            float maxPointsPart = Odometry::DEFAULT_MAX_POINTS_PART(),
            const std::vector<int> &iterCounts = std::vector<int>(),
...
CV_WRAP static Ptr<ICPOdometry>
...
CV_WRAP Ptr<RgbdNormals>

getNormalsComputer() const {
...
class CV_EXPORTS_W RgbdICPOdometry
 ...
 * @param transformType Class of trasformation
 */
RgbdICPOdometry(const Mat &cameraMatrix, float minDepth = Odometry::DEFAULT_MIN_DEPTH(),
                float maxDepth = Odometry::DEFAULT_MAX_DEPTH(),
                float maxDepthDiff = Odometry::DEFAULT_MAX_DEPTH_DIFF(),
                float maxPointsPart = Odometry::DEFAULT_MAX_POINTS_PART(),
                const std::vector<int> &iterCounts = std::vector<int>(),
...
CV_WRAP static Ptr<RgbdICPOdometry>
 ...
 */
class CV_EXPORTS_W FastICPOdometry
...
CV_WRAP static Ptr<FastICPOdometry>

create(const Mat &cameraMatrix,
       float maxDistDiff = Odometry::DEFAULT_MAX_DEPTH_DIFF(),
       float angleThreshold = (float) (30. * CV_PI / 180.),
       float sigmaDepth = 0.04f,
       float sigmaSpatial = 4.5f,
...
CV_EXPORTS_W
void
warpFrame(const Mat &image, const Mat &depth, const Mat &mask, const Mat &Rt,
          const Mat &cameraMatrix,
          const Mat &distCoeff, OutputArray warpedImage, OutputArray warpedDepth = noArray(),
